...
const ProductList = ({
  ...
  <div className="rounded border shadow bg-white p-2 mb-3 d-flex flex-column gap-2">
    ...
    <div className="col-lg-12 col-6 mt-2">
      ...
      {services === null ? (
        <p className="m-2">Layanan Tidak Tersedia</p>
      ) : services?.length > 0 ? (
        <select
          name="shipping"
          id="shipping"
        ...
        >
          ...
          {services?.map((item, i) => (
            <option key={i} value={item.cost}>{`Rp ${parseFloat(
              item.cost
            ).toLocaleString("id-ID")} - ${item.etd} - ${
              item.description
            }`}</option>
          ))}
        </select>
    ...
    <table className="table">
      <tbody>
        ...
        <tr>
          <td>Ongkir</td>
          <td>:</td>
          <td>{`Rp ${parseFloat(service ? service : 0).toLocaleString(
            "id-ID"
          )}`}</td>
        </tr>
        <tr>
...
const Checkout = () => {
  ...
  const [
    createOrder,
    { data: token, isLoading: tokenLoad, error, isSuccess, reset },
      ...
      prevProducts.map((product) => {
        if (product.id === id) {
          ...
          return {
            ...
            weight: newQuantity * product.weight,
          };
        }
        return product;
      })
